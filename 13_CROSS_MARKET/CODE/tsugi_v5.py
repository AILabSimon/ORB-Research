#!/usr/bin/env python3
"""TSUGI v5 (causal window) — adds the EDGE-INTERACTION (straddle) hypothesis.
...
def build(inst, tz="America/New_York", r0=R0):
    ...
    for day,g in df.groupby("day",sort=True):
        ...
        # FIXED LOOKBACK windows ending AT the entry bar -- never degenerate,
        # always strictly pre-entry information, so a rule built on them is tradeable.
        # STRICTLY CAUSAL: the window ends at ei-1. The entry bar is EXCLUDED because
        # its body needs its close, which is not known at the moment the retest fires.
        for K in (10,20,30):
            e1=ei-1
            if e1<0: continue
            k0=max(e1-K+1,0)
            fx=edge_features(po[k0:e1+1],ph[k0:e1+1],pl[k0:e1+1],pc[k0:e1+1],edge,dr)
            res.update({f"lb{K}_{k}":v for k,v in fx.items()})
        j2=min(ei+16,len(pm))
